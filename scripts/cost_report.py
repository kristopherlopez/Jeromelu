...
def fetch_latest_daily_rate(today: date) -> float:
    """Returns yesterday's full-day spend — the proxy for the *current*
    daily run-rate. Used by the projection so a cost-cutting fix today
    starts pulling the forecast down within a day or two (instead of the
    historical-average projection which would drag the entire month's
    pre-fix spend through the rest of the forecast).

    Cost Explorer's ~24h lag means today's bucket is usually incomplete,
    so we deliberately look at *yesterday*. Day 1 of the month falls back
    to 0 since there's no in-month "yesterday" to read.
    """
    if today.day == 1:
        return 0.0
    ce = boto3.client("ce", region_name="us-east-1")
...
def project_month_end(mtd: float, daily_rate: float, today: date) -> float:
    """Projection = MTD + (current daily rate * days remaining in month).

    Uses the latest complete day as the run-rate proxy rather than the
    month-to-date average. Recovers quickly from large day-over-day step
    changes (e.g. a scale-to-zero fix lands → projection drops in 1-2
    days instead of being dragged down by a month of historical spend).
    """
    days_in_month = calendar.monthrange(today.year, today.month)[1]
    remaining_days = max(0, days_in_month - today.day)
    return mtd + daily_rate * remaining_days
...
<table style="border-collapse:collapse">
  ...
  <tr><td style="padding:2px 16px 2px 0">Yesterday (run-rate)</td><td>{_fmt_money(daily_rate)}</td></tr>
  <tr><td style="padding:2px 16px 2px 0">Projected EOM</td><td>{_fmt_money(projected)} <span style="color:#888">= MTD + ({_fmt_money(daily_rate)} &times; {days_remaining} days)</span></td></tr>
...
<p style="color:#888;font-size:12px;margin-top:24px">
  ...
  Projection uses yesterday's spend as a flat run-rate. The figure stabilises a couple of days after any major config change.
</p>
</body></html>
"""
...
def main() -> int:
    ...
    daily_rate = fetch_latest_daily_rate(today)
    projected = project_month_end(mtd_total, daily_rate, today)
