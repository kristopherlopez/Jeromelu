r"""Generic Scout backfill driver.
...
S3_KEY_FN: dict[str, Callable[[int, int, int | None], str]] = {
    "nrlcom-draw": lambda comp, season, rd: (
        f"scout/nrlcom/draw/{comp}/{season}/round-{int(rd or 0):02d}.json"
    ),
    "nrlcom-ladder": lambda comp, season, rd: (
        f"scout/nrlcom/ladder/{comp}/{season}/round-{int(rd or 0):02d}.json"
    ),
    "nrlcom-stats": lambda comp, season, rd: (
        f"scout/nrlcom/stats/{comp}/{season}.json"
    ),
    "supercoach-stats": lambda comp, season, rd: (
        f"scout/nrlsupercoachstats/stats/{season}/round-{int(rd or 0):02d}.json"
    ),
}
...
S3_LIST_PREFIX_FN: dict[str, Callable[[int, int, int | None], str]] = {
    "nrlcom-match-centre": lambda comp, season, rd: (
        f"scout/nrlcom/match-centre/{comp}/{season}/round-{int(rd or 0):02d}/"
    ),
}
...
def _s3_key_exists(client, *, bucket: str, key: str) -> bool:
    ...
    try:
        ...
        return True
    except Exception:  # noqa: BLE001 — botocore ClientError 404, etc.
        return False
    ...
    with httpx.Client(timeout=300.0) as client:
        for season in seasons:
            for rd in rounds:
                ...
                except Exception as e:  # noqa: BLE001 — surface any HTTP/network err
