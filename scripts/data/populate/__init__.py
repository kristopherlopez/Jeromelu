...
Each module is one extractor that reads from `s3://jeromelu-clean-documents/scout/...`
...
Phases (run in this order to respect FK dependencies):
  ...
  1. identity   — backfill people.nrlcom_player_id + teams.nrlcom_team_id
                  from scout/nrlcom/match-centre/ archives
  2. rounds     — extract_rounds from scout/nrlcom/draw/
  3. matches    — extract_matches from scout/nrlcom/match-centre/
  ...
  7. standings  — extract_team_standings from scout/nrlcom/ladder/
  8. leaderboards — extract_stat_leaderboards from scout/nrlcom/stats/
