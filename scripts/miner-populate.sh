#!/usr/bin/env bash
# Run Miner S3-to-DB populate phases inside the prod API container.
#
# This is the scheduled bridge for pipelines whose admin endpoint captures raw
# Miner archives to S3 while the relational projection lives in
...
# appends status lines plus Python output to /var/log/jeromelu/miner-populate.log.
#
# Usage:
#   miner-populate.sh nrlcom-current [--seasons 2026] [--competition 111]
#   miner-populate.sh phase leaderboards [--seasons 2026] [--dry-run]
#   miner-populate.sh all [--seasons 2026] [--dry-run]
#   miner-populate.sh nrlcom-current --no-op
...
usage() {
  cat <<'USAGE'
Usage:
  miner-populate.sh nrlcom-current [--seasons YEAR[,YEAR...]] [--competition N] [--dry-run] [--no-op]
  miner-populate.sh phase PHASE [--seasons YEAR[,YEAR...]] [--competition N] [--dry-run] [--no-op]
  miner-populate.sh all [--seasons YEAR[,YEAR...]] [--competition N] [--dry-run] [--no-op]
...
Environment overrides:
  MINER_API_CONTAINER      Container name, default jeromelu-api.
  MINER_HOST_ROOT          Host checkout root, default /opt/jeromelu.
  MINER_LOG_DIR            Log directory, default /var/log/jeromelu.
  MINER_POPULATE_RUNTMP    Container staging dir, default /runtmp/miner-populate.
USAGE
}

die() {
  echo "miner-populate: $*" >&2
  exit 2
}
...
API_CONTAINER="${MINER_API_CONTAINER:-jeromelu-api}"
HOST_ROOT="${MINER_HOST_ROOT:-/opt/jeromelu}"
LOG_DIR="${MINER_LOG_DIR:-/var/log/jeromelu}"
RUNTMP="${MINER_POPULATE_RUNTMP:-/runtmp/miner-populate}"
...
case "$RUNTMP" in
  /runtmp/miner-populate|/runtmp/miner-populate/*) ;;
  *) die "MINER_POPULATE_RUNTMP must stay under /runtmp/miner-populate" ;;
esac

PYTHONPATH_VALUE="${RUNTMP}/packages/shared:${RUNTMP}"
LOG_FILE="${LOG_DIR}/miner-populate.log"
DRY_RUN_ARGS=()
