...
def render_markdown(
    bucket: str, prefix: str, sample_keys: list[str], rows: list[dict[str, Any]]
) -> str:
    out: list[str] = []
    out.append("---")
    ...
    out.append("")
    out.append(
        f"_Generated by `scripts/profile_s3_json.py`. Bucket: `{bucket}`. "
        f"Sample size: {len(sample_keys)} objects._"
    )
    out.append("")
    ...
    for r in rows:
        ...
        out.append(
            f"| `{path_str}` | {r['type']} | {r['presence']} | "
            f"{r['nullable']} | {r['cardinality']} | {ex} |"
        )
    out.append("")
    return "\n".join(out)
