...
The Ask Me feature lets users chat with JeromeLu about NRL SuperCoach strategy. It uses a curated knowledge base rather than raw transcript chunks.
...
Build context from top-K KB entries
    |
    v
LLM call (gpt-4o) with JeromeLu system prompt + context
    |
    v
...
| `decision` | JeromeLu's past trades/captain picks + outcomes | Events (action/prediction type) |
| `opinion` | JeromeLu's opinionated stance on a player | Aggregated claim sentiment |
