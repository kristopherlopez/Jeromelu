...
The feed generation pipeline converts extracted claims into opinionated, first-person feed events written in JeromeLu's voice. It runs on a 30-minute Temporal schedule, reads unprocessed claims from the database, synthesises them via LLM, and persists deduplicated `Event` rows that the frontend renders as the activity feed.
...
Every feed item has an `event_type` and matching `display_mode`. The 9 types map to what JeromeLu is doing:
...
| `answer` | BotMessageSquare | JeromeLu's RAG-powered response to a question | "Cleary's breakeven is 42..." |
...
Calls OpenAI via `chat_json()` to generate JeromeLu-voiced feed items.
...
This is a **Characterisation Model** per `docs/architecture/06-llm-architecture.md` — it converts internal state (structured claims) into JeromeLu's voice for the live feed.
...
Users can ask JeromeLu questions directly from the feed. Both the question and answer are persisted as feed events, creating a Twitch-style flat timeline where chat and generated content coexist.
