...
> This doc is archived — the landing page is no longer the default entry. Nav-bubble targets above reflect the current live nav on inner pages, which persisted when the landing page was retired. Historical nav (My Squad, The Dossier) is preserved in [architecture/03-experience-architecture.md](../architecture/03-experience-architecture.md).
