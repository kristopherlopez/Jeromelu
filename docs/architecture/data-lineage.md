...
| `position` (eligibility) | SC `players-cf` `positions[0]` (primary), `positions[]` (all eligible) | SC archive | `scout/supercoach_roster/` (existing) | `people_attributes.primary_position` (SCD-2); full list in `people_attributes.metadata_json.eligible_positions` |
| `team` (SCD-2) | SC `players-cf` `team.abbrev` → slug lookup | SC archive | `scout/supercoach_roster/` | `people_attributes.team_id` (SCD-2 close-current-open-new on diff) |
