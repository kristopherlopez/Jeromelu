...
| **A show worth following** | Return rate concentrated at episode beats (Monday and Thursday peaks visible in traffic). Session depth on Thursdays (The Call beat). | Viewers refer to Jaromelu by name in inbound traffic / share copy. The crew is named (Miner, Analyst) in user language unprompted. |
