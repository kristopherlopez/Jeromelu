...
| Jeromelu | The front man. Confident posture. Leans toward camera. Orange accent in visual design. | Confident, smirking, grimacing, shrugging, thinking, pointing at camera |
...
| Remark card (Jeromelu) | 48px circle |
| Drill-down panel header | 64px circle |
| Receipt card | 80px |
| Face-off layout (Critic vs Jeromelu) | 48px each |
...
|--------|------|----------|
| High conviction Remark entry | Jeromelu leaning in, confident | 3-5s |
| Critic confrontation | Split-screen: Critic sceptical, Jeromelu defiant | 5-8s |
| Resolution — correct | Jeromelu smirking, leaning back | 3-5s |
| Resolution — wrong | Jeromelu grimacing, shrugging | 3-5s |
| Resolution — spectacular miss | Jeromelu head in hands | 5-8s |
| Crowd callout | Jeromelu pointing at camera | 2-3s |
...
| Jeromelu | 🎤 | The front man. Makes the call. |
...
Show only active crew + Jeromelu. Dormant collapse to `+3` overflow pill. Tap to expand as dropdown.

### All Dormant

When no crew is working:
```
[○] Scout  [○] Analyst  [○] Critic  [○] Bookkeeper  [○] Archivist  [●] Jeromelu — Squad locked. Watching the market.
```

Jeromelu is never fully dormant.
...
Large, prominent. The hero content. Always attributed to Jeromelu.

**Open State:**

For high conviction Remarks, a video clip plays inline above the card before it appears: `[VIDEO: Jeromelu leaning in, confident, 3-5s]`
...
- "JEROMELU" label: uppercase, zinc-400, Geist Mono, small
...
- Postmortem from Jeromelu in italics: "Told you. The Storm matchup was a trap."
...
User question paired with Jeromelu's response.
...
- Jeromelu section: orange left border, italic voice text
...
**Beat divider:**
```
─── THE CALL · Jeromelu is making his call ───
```
...
| Tap... | Opens... |
|--------|----------|
| Player name (e.g., "Cleary") | Player Dossier panel |
| Expert name (e.g., "KingOfSC") | Expert Dossier panel |
| Squad reference | Squad panel |
...
│  ── Recent Trades ──         │
│  Gutho OUT → Mam IN         │
│  "I moved Gutho because..." │
│                              │
│  ── The Plan ──              │
│  Watching: Hynes, Munster   │
│  Considering: Cleary → Hynes│
│                              │
└──────────────────────────────┘
```
...
```
[●] Scout — Scanning 6 new episodes    [●] Analyst — Cross-referencing Cleary claims    [○] Critic    [○] Bookkeeper    [○] Archivist    [●] Jeromelu — Reviewing the evidence
```
...
Jeromelu: "Three pods this week all selling. The matchup is awful. When everyone's running for the exit, I don't stand in the doorway."

User: "Your Munster call looks shaky after that injury report."
Jeromelu: "It's a cork. He's played through worse. The Analyst confirmed he trained fully today. Conviction unchanged."
```
...
| Tap player/expert name | Dossier panel slides in |
...
| Type in Interaction Bar | Ask Jeromelu a question, response appears as Dialogue Card |
...
- **Jeromelu's avatar should appear alongside every Remark** — he's the character making the call.
...
- **Typing indicator**: When generating a response, show Jeromelu's avatar in a "thinking" pose, not three bouncing dots.
- **Idle state**: When no content is active, Jeromelu's avatar should still be animated (thinking, reviewing). The site should never feel empty.
