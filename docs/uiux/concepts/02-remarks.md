...
A Remark is an opinionated, voiced analytical piece that Jeromelu puts his name on publicly. It is built from the crew's resolved knowledge — claims cross-referenced, contradictions weighed, evidence assembled — and delivered in Jeromelu's voice.
...
A Remark doesn't just appear in the Stream. It **arrives**. Jeromelu is making a public call, putting his reputation on the line. The UI should make that feel like a moment, not a card rendering.

### Entry Animation

When a new Remark enters the Stream:

1. **Jeromelu's avatar slides in** from the left — full size (~48px), full colour.
2. For **high conviction Remarks**: A short **video clip** (3-5 seconds) plays inline first — Jeromelu leaning in, looking at camera, confident. The clip is from the reusable reaction library, not generated per Remark.
3. The **voice text types out** phrase by phrase — fast enough to feel punchy (not tedious character-by-character). Think: 2-3 words appearing at a time, full text visible in ~3 seconds. This creates the sensation of Jeromelu *speaking*, not text *rendering*.
...
- **High**: Video clip plays first. Avatar is larger. Text types out with slightly more dramatic pacing. The card has a subtle glow. The Crew Bar briefly highlights Jeromelu. **This is the moment.**
...
| Element | Description |
|---------|-------------|
| **Avatar** | Jeromelu's character portrait (~48px). Plays a subtle idle loop within the card. For high conviction: slightly larger with orange glow ring. |
| **Attribution** | "JEROMELU" alongside the avatar. Top-left. |
...
| **Subject tag** | The player, team, or matchup the Remark is about. Tappable — opens Dossier panel. |
...
- Jeromelu's avatar plays a confident idle loop within the card — slight lean, occasional glance at camera
...
**Visual:**
- **Lock animation**: A brief visual "seal" effect plays — a lock icon stamps onto the card, the orange border shifts to amber, the card slightly darkens. Jeromelu's avatar shifts expression to a calm, committed look. The Remark has been committed.
...
2. A short **video clip** (3-5 seconds) of Jeromelu plays — smirking, leaning back, the victor. From the reusable clip library.
...
2. A short **video clip** of Jeromelu — grimacing, shrugging, owning it. Self-deprecating but not defeated.
...
- A brief amber tint. Jeromelu's avatar shows a "hmm, close" expression.
- Status badge: `~ MIXED` in zinc/grey
- Jeromelu's take on why it's a push
...
▾ Evidence trail
  ...
  "Last time Jeromelu sold Cleary against consensus:
   Round 3. Cleary scored 38. Correct call."

  [👤] Jeromelu · Thu 11:00 AM
...
The receipt card features **Jeromelu's character portrait** prominently — this is a meme-format shareable designed for group chats.
...
Jeromelu's avatar expression matches the result: smirk for correct, grimace/shrug for wrong.
...
Both correct and wrong receipts are designed as **meme-format shareables** — Jeromelu's face + the call + the result. No additional context needed. A group chat member who has never visited the site understands it instantly.
...
- Pre-game (Open state): "Jeromelu is calling SELL on Cleary. Do you agree?"
- Post-game (Resolved state): "Jeromelu called it." / "Jeromelu missed."
- Copy link, or share directly to platform.

### Tap Subject

- Tapping a player/team/matchup name opens the Dossier drill-down panel.
- The Dossier shows everything the crew knows about that entity.
...
- Questions are contextual — Jeromelu knows which Remark the user is referring to.
