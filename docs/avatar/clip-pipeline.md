...
Jaromelu's avatar is a looping video that reacts to system events and user interactions. It's powered by a library of short video clips, a JSON manifest that describes them, and a frontend state machine that sequences them in real time.
...
Talking clips are generated when Jaromelu has something specific to say — a prediction call, a bold take, a weekly recap line. Unlike silent reaction clips, these include lip-synced speech driven by a text script.
...
| `ConnectedAvatar` | `components/ConnectedAvatar.tsx` | Reads current clip from engine, passes to `JaromeluAvatar`. |
| `JaromeluAvatar` | `components/JaromeluAvatar.tsx` | Renders dual `<video>` elements with crossfade transitions. |
...
| `services/web/src/app/components/JaromeluAvatar.tsx` | Video player with crossfade |
