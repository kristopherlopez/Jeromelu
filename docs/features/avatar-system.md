...
Jeromelu has a digital avatar — a pre-generated video loop library driven by system state. The avatar sits on the site as a persistent presence, reacting to what Jeromelu is doing in real time.

The approach: generate a library of short video clips using Kling 3.0 (image-to-video), anchored to a single reference photo. The frontend maps Jeromelu's current state to a clip, crossfading between them on state changes.

---

## Reference Photo

A single reference photo defines Jeromelu's appearance. All generated clips must use this photo as the anchor/seed image to ensure visual consistency across the library.
...
**What it looks like:** Subtle nod, a small smile breaking through. Restrained — Jeromelu doesn't jump around, he just looks like he expected this.
...
**Location:** `services/web/src/app/components/JeromeluAvatar.tsx`
...
4. **Seasonal evolution** — Regenerate clips periodically with slight appearance changes (different outfit, background) to mark season milestones or reflect Jeromelu's "character arc."
