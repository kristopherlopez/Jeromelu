...
- `GET /api/sources/{source_id}/voice-clusters` — aggregates `source_speakers` by `speaker_label` (skipping NULL labels) and returns `{speakers: [{speaker_label, turn_count, total_seconds, first_ts, last_ts, embedding_eligible_count, dominant_person_id, dominant_person_name, dominant_share, match_method_breakdown, turns: [...]}]}`. Sorted by `total_seconds` descending. `turns` is *every* turn in the cluster, chronologically, each carrying `{segment_id, start_ts, end_ts, duration, speaker_person_id, match_method, has_embedding, preview_text}` — `preview_text` is the full concatenated chunk text of the turn (soft-truncated at ~300 chars). `dominant_person_*` is the mode of `speaker_person_id` across the cluster's turns; the `match_method_breakdown` shows how each turn was attributed so the operator can see at a glance whether the cluster is currently `voice+face` confirmed, face-only, or unattributed.
