...
| | |
|---|---|
| **Package** | `services/api/app/miner/presenter_research/agent.py` |
| **Trigger** | `make miner-presenters CHANNEL_ID=…` (CLI) or `POST /api/admin/presenters/research/{channel_id}` (admin UI) |
| **Crew counterpart** | [Miner](../crew/miner/README.md) — research/discovery mode, scoped to one channel |
...
Given one channel (a YouTube channel, podcast, or website), Presenter Miner researches who the regular presenters are via Anthropic-hosted web tools, files findings into `miner_presenter_candidates`, and stops. Reviewers confirm/reject in the admin "Presenters" tab; confirmation creates a `people` row (or links to an existing one) and writes a `source_presenters` association.
...
`agent_audit` plumbing is identical to the main Miner — same `agent_runs` row, same per-event `agent_events`, same JSONL forensic upload to `s3://jeromelu-clean-documents/agent-logs/presenter_miner/…`.
...
| File | Purpose |
|---|---|
| `app/miner/presenter_research/agent.py` | Tool definitions + handlers + system prompt + run loop |
| `app/miner/presenter_research/cli.py` | `make miner-presenters` entry point; accepts `--channel-id` or `--source-id` |
...
| Table | Role |
|---|---|
| `miner_presenter_candidates` | Staging inbox. Agent writes here; status `pending` → `confirmed` / `rejected` on review. |
| `source_presenters` | Confirmed `(channel_id, person_id, role)` association. Created on confirm. |

Schema details: [data-catalogue: miner_presenter_candidates](../../operations/data-catalogue/miner_presenter_candidates.md). Migration: `052_source_presenters.sql`.

---

## Bounds

Tighter than the discovery Miner — one channel is a small task, a runaway loop is a bug.
...
**Outputs:**
- ≥0 rows in `miner_presenter_candidates` with `status='pending'`. Each row carries:
...
- One `agent_runs` row keyed on `run_id` with `agent_id='presenter_miner'`, full token + cost rollup.
- One JSONL forensic bundle at `s3://jeromelu-clean-documents/agent-logs/presenter_miner/{YYYY}/{MM}/{DD}/{run_id}.jsonl`.
...
CLI:
```bash
make miner-presenters CHANNEL_ID=<uuid>
make miner-presenters SOURCE_ID=<uuid>          # resolves to channel server-side
make miner-presenters CHANNEL_ID=<uuid> DRY_RUN=1
make miner-presenters CHANNEL_ID=<uuid> MODEL=claude-opus-4-7
```

Admin UI: `/admin` → **Presenters** tab → pick channel → **Run Presenter Miner**. Synchronous (~30s); spinner while it runs.
...
- **Auto-trigger on first ingestion.** Last step of `make collect-audio` for any new source whose channel has zero confirmed presenters: queue a Presenter Miner run in the background. Reviewer wakes up to a populated inbox.
...
- [Miner (source discovery)](source-discovery.md) — sibling agent; same loop shape, different scope
