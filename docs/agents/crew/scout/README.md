...
| **Scope**             | Media discovery + enumeration + raw transcript pull · SuperCoach roster + stats · NRL.com matches / team lists / injuries / rounds · future: podcasts, radio, Twitter/X, Instagram, blogs, Reddit |
...
| **Platform coverage** | Media: YouTube only today; podcasts/RSS/radio/Twitter/Instagram/blogs/Reddit on backlog. Data: SuperCoach API + NRL.com endpoints land in the charter rollout.            |
...
1. **Discovering new channels** across platforms — deterministic YouTube-native search ([architecture.md §3.1](architecture.md), in design) for the bulk case; agentic web hunt today ([§3.2](architecture.md), shipped) for off-platform / long-tail. YouTube only today; podcasts / radio / Twitter / Instagram / blogs / Reddit on backlog.
...
7. **Multi-platform expansion** — instantiate the same shape (discovery → approval → enumerate → refresh → extract) for podcasts (RSS), radio, Twitter/X, Instagram, blogs/news, Reddit. *Backlog* — schema is platform-agnostic; code is YouTube-only.
