...
1. **Discovering new channels** across platforms — deterministic YouTube-native search ([architecture.md §3.1](architecture.md), in design) for the bulk case; agentic web hunt today ([§3.2](architecture.md), shipped) for off-platform / long-tail. YouTube only today; podcasts / Twitter / blogs / Reddit on backlog.
