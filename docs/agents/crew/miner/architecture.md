...
How Miner works in depth: where it sits in the pipeline, what it writes (the hand-off contract), the discovery and tracked-source flows, the component internals, and the architecture under the expanded charter. For Miner's identity, scope, and voice see [README.md](README.md); for the locked design decisions see [charter.md](charter.md); for status and roadmap see [roadmap.md](roadmap.md).

---

## Pipeline position

Miner is the **inventory stage** of a multi-agent pipeline. It owns everything from "we don't know about this source" through "raw transcripts in the database." Parsing, scoring, numbers, and synthesis are downstream.
...
Miner's outputs are raw inventory rows only — Extract + Load, never Transform. The full chain Miner owns spans **media** (`miner_candidates → channels → sources → audio`) and **data** (`people / player_attributes / player_rounds / matches / match_team_lists / injuries / rounds`).

**Media writes:**

| Table | What Miner writes | What Miner does **not** write |
|---|---|---|
| `miner_candidates` | Full row at discovery (kind, title, score, content_categories, score_reasons, run_id, `status='pending'`) | — |
...
| Table | What Miner writes | Module folder | Phase |
|---|---|---|---|
| `people` | Roster upsert from SC `players-cf` | `miner/supercoach_roster/` | 1 ✅ |
| `player_attributes` | Position / team / contract (SCD-2); enriched by NRL.com profile data | `miner/supercoach_roster/` + `miner/nrlcom_players_roster/` | 1 ✅ / 4.5 |
| `people_roles` | Primary role tenure (SCD-2) | `miner/supercoach_roster/` | 1 ✅ |
| `claims` + `quotes` (from SC `notes[]`) | SC editorial commentary on players | `miner/supercoach_roster/` (extractor) | 2.5 |
| `teams.metadata_json.supercoach` | SC team IDs cross-reference | `miner/supercoach_teams/` | 2.5 |
| `sc_settings` (new) | SC game rules per season | `miner/supercoach_settings/` | 2.5 |
| `player_rounds` | Per-round stats — D11 merge of nrlcom match-centre + nrlsupercoachstats jqGrid | `miner/supercoach_stats/` + `miner/nrlcom_match_centre/` (extractor merges) | 2 ✅ / 3 |
| `matches` | Fixtures, results, score, venue, attendance, weather | `miner/nrlcom_draw/` + `miner/nrlcom_match_centre/` (extractor) | 3 |
| `match_team_lists` | Lineups from match-centre `positionGroups` + `players[]` | `miner/nrlcom_match_centre/` (extractor) | 3 |
| `player_match_stats` (new) | Per-player per-match 58-field stat line | `miner/nrlcom_match_centre/` (extractor) | 3 |
| `match_timeline` (new) | 100+ typed timeline events per match | `miner/nrlcom_match_centre/` (extractor) | 3 |
| `match_officials` (new) | Referee + touch judges + bunker per match | `miner/nrlcom_match_centre/` (extractor) | 3 |
| `rounds` | Round metadata derived from draw | `miner/nrlcom_draw/` (extractor) | 3 |
| `injuries` | Official casualty-ward state | `miner/nrlcom_casualty_ward/` | 4 |
| `team_standings` (new) | Ladder positions + 22 per-team metrics | `miner/nrlcom_ladder/` | 4 |
| `stat_leaderboards` (new) | Pre-computed top-25 leaderboards | `miner/nrlcom_stats/` | 4.5 |

Miner writes **nothing** to `source_documents`, `source_speakers`, `source_chunks` (Analyst's transcription writes), `source_chapters`, `source_annotations`, `quotes`, `claims`, `claim_chunks`, `claim_associations`, `consensus_snapshots`, `predictions`, `decisions`, `wiki_pages`, or any reasoning/output table. If a Miner-voiced UI line mentions parsed content (e.g. *"deep dive on Munster"*), that content was generated by a downstream agent and is being *surfaced through* Miner's voice mode — not produced by Miner itself.

---

## Flow

The full Miner function decomposes into two phases joined by a human-approval gate. The two phases answer two unrelated questions, so each gets its own diagram.
...
Both surfaces file into the same `miner_candidates` table. Human approval is the seam where LLM judgement stops and idempotent pipelines take over.

### Discovery — *how do candidates land in `miner_candidates`?*
...
flowchart LR
    ...
    subgraph Ag["Agentic surface — shipped"]
      AgJob["§3.2<br/>Miner LLM loop<br/>web_search · web_fetch<br/>dedupe_check_bulk"]
    end

    Pending[("miner_candidates<br/>status='pending'")]
...
**Trace:**
1. **Deterministic YouTube sweep** (§3.1, shipped) — the admin endpoint / CLI runs channel search, video search, video-harvest channel discovery, and optional related-channel walks. The server loads known IDs from `channels`, `sources`, and `miner_candidates`, passes them into the YouTube helper filters, scores enriched results with metadata-backed reasons, and persists novel candidates.
2. **Weekly agentic sweep** (§3.2) — Miner LLM loop runs on a slower cadence with a brief that says *"deterministic covers YouTube-native; your job is off-platform reach and the long tail."* Uses `web_search` for blog/news/Reddit mentions, `web_fetch` to read About pages and judge quality, persists what survives.
...
flowchart LR
    ...
    subgraph AudioStep["Miner § 3.5 — audio acquisition"]
...
2. **Daily cron keeps things current** (§3.4) — `POST /admin/miner/refresh-videos` walks each channel for new uploads using the last known `video_id` as cursor (typically 1 quota unit / channel / day) and re-snapshots stats for every YouTube source. ~750 quota units / pass.
...
- Read-side: `channels.external_id`, `sources.canonical_url`, `miner_candidates.(platform,kind,external_id)` for the server-side filter
...
5. Persist novel results directly to `miner_candidates` with `status='pending'`. Idempotent on `(platform, kind, external_id)`.

**Outputs** — rows in `miner_candidates` (same table as §3.2, distinguished by `discovered_via='deterministic_youtube:...'`) with score, score reasons, categories, and useful metadata captured at discovery.
...
**Audit** — the admin endpoint writes `agent_runs` rows through the standard deterministic Miner pattern: `agent_id='miner'`, `model='deterministic'`, and `detail_json.pipeline='youtube-discovery'`.

### 3.2 Discovery agent `[agentic]`

The web-hunting LLM loop. Files candidate channels and videos to `miner_candidates`. With deterministic discovery shipped, this is the long-tail / off-platform surface.

**Trigger** — Manual CLI: `python -m app.miner.source_discovery.cli` (flags: `--dry-run`, `--max-turns`, `--budget`, `--brief`). Scheduled runs are planned.

**Inputs**
- System prompt (cacheable, ~1.1k tokens) — Miner voice + scope + tagging taxonomy
...
| `dedupe_check_bulk` | Custom | Batched front-door firewall against `channels` / `sources` / `miner_candidates` |
| `dedupe_check` | Custom | Single-item variant for one-off investigations |
| `persist_candidate` | Custom | Idempotent INSERT into `miner_candidates` (`status='pending'`) |

**Processing** — multi-turn streaming loop in `services/api/app/miner/source_discovery/agent.py`. Each turn: assistant emits text + tool calls; client-side handlers run dedupe / persist; server-side tools execute on Anthropic's side. Loop ends on `end_turn` or first bound hit.

**Outputs** — rows in `miner_candidates` (kind, title, score, content_categories, score_reasons, run_id, `status='pending'`, `discovered_via='web_search'`). Console theatre: per-turn text streamed live, one line per tool call.
...
- S3 JSONL bundle — `agent-logs/miner/{YYYY}/{MM}/{DD}/{run_id}.jsonl` on `jeromelu-clean-documents`
...
**Trigger** — admin approve action on a `miner_candidates` candidate of `kind='channel'`, via `app/routers/recon.py`.
...
**Failure mode** — approval still commits if YouTube API fails; channel is in `channels`, admin can re-trigger via the per-channel endpoint (§3.4 — `POST /admin/miner/channels/{ref}/refresh-videos?full_backfill=true`) without waiting for the daily cron.

**Audit** — one `agent_runs` row under `agent_id='miner'`, `detail_json.pipeline='youtube-channel-videos'`. The run detail records the channel identity, `full_backfill=true`, `max_results`, and inserted/listed/metric counts. Approval still commits if this audited backfill fails; the failure row is the recovery breadcrumb for the per-channel endpoint.
...
**Trigger** — `POST /api/admin/miner/refresh-videos` with `X-Admin-Key`. Optional `?skip_stats=true` or `?skip_enumerate=true`. Cron suggestion: Mon 09:00 AET.
...
**Channel stats variant** — `POST /api/admin/miner/refresh-channel-stats` snapshots channel-level metadata (`channel_metrics`: subscribers, total views, video count, country, channel age, handle/avatar sync). It is separate from the heavier video refresh so channel popularity still updates if video enumeration fails.

**Audit** — `POST /api/admin/miner/refresh-videos` writes one `agent_runs` row under `agent_id='miner'`, `detail_json.pipeline='youtube-refresh-videos'`, and returns that `run_id` alongside the existing `enumerate`/`stats` payloads. The per-channel ad-hoc variant writes `detail_json.pipeline='youtube-channel-videos'`; the channel-stats endpoint writes `detail_json.pipeline='youtube-channel-stats'`. Failures mark the corresponding row `failed` before propagating the HTTP error.

### 3.5 Audio acquisition `[deterministic, shipped]`

Miner's last Extract step. Pulls audio from approved-but-pending YouTube sources and lands it in S3. **Extract only** — does not interpret the audio. Transcription / diarisation belongs to [Analyst](../analyst/README.md) (see [analyst/transcription](../../system/transcription-pipeline.md)).

**Trigger** — Manual CLI: `python -m app.miner.media.cli.audio <source_id>` or `make collect-audio SOURCE_ID=<uuid>`. The recurring drain job (APScheduler / cron over `ingestion_status='pending'`) is on the backlog.

**Module** — `services/api/app/miner/media/audio.py` · `acquire_audio(session, source)`.
...
**Hand-off boundary** — Miner is done when the source row has `ingestion_status='collected'` and `audio_s3_key` set. Analyst picks it up from there:
...
Miner now has a route-free internal health layer in `app.miner.source_health`.
...
**Inputs**
- `agent_runs` rows where `agent_id='miner'` and
...
The expanded charter (see [charter.md](charter.md)) generalises this media-side shape to all external data acquisition. One Miner identity, many modules; raw JSON captured to S3 first, then projected into DB tables by downstream extractors.
...
External world
       ...
       ▼
┌─────────────────────────────────────────────────────────┐
│ Miner (one agent identity, many modules)                │
│                                                         │
│  Media (miner/media/ + youtube/, source_discovery/):    │
│    • discovery (loop.py + refresh.py)                   │
│    • audio acquisition (audio.py)                       │
│    • metadata refresh                                   │
│                                                         │
│  Data — supercoach.com.au (folder per pipeline, D9):    │
│    • miner/supercoach_roster/                  shipped  │
│    • miner/supercoach_teams/                   new      │
│    • miner/supercoach_settings/                new      │
│    • miner/supercoach_draft_*/                 optional │
│                                                         │
│  Data — nrl.com (canonical per D11):                    │
│    • miner/nrlcom_draw/                        new      │
│    • miner/nrlcom_match_centre/                new ★    │
│    • miner/nrlcom_casualty_ward/               new      │
│    • miner/nrlcom_ladder/                      new      │
│    • miner/nrlcom_stats/                       new      │
│    • miner/nrlcom_players_roster/              partial  │
│                                                         │
│  Data — nrlsupercoachstats.com:                         │
│    • miner/supercoach_stats/                   shipped  │
└─────────────────────────────────────────────────────────┘
       │
       ▼ (S3-first per D10)
┌─────────────────────────────────────────────────────────┐
...
**Shared shape across all Miner modules:**
...
2. Each module gets an admin endpoint (`POST /api/admin/miner/<pipeline>`) that wraps the function in the `agent_runs` audit pattern (`record_agent_started/ended`).
3. Each module's wrapper writes one `agent_runs` row with `agent_id='miner'`, `detail_json.pipeline='<module>'`, plus per-run counts (rows fetched, rows upserted, rows skipped, errors).
...
6. `GET /api/admin/miner/dashboard` is the read-only operator rollup over those
   rows: it filters to `agent_id='miner'`, groups recent runs by
...
This is the pattern Miner's media side already follows; the expansion just instantiates it for more modules.
...
- [Data lineage](../../../architecture/data-lineage.md) — end-to-end source → S3 → DB → app map for every Miner pipeline output
