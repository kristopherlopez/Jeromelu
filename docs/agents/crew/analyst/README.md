...
1. **Cleaning, referential resolution, embedding, entity / quote / claim extraction (with falsifiable-prediction capture), cross-source consensus + contradiction detection** — Analyst's durable scope. Resolves what the words point to, cross-references claims across sources, finds contradictions, detects consensus shifts, and makes each prediction *gradeable* for the ledger. **No production pipeline yet** — prototyped locally, built as **worker** passes per [roadmap Track 2](roadmap.md#track-2--interpretive-pass-buildout) (skills are local-only — [charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills)).
...
| **Scope (durable)**   | The interpretive layer: cleaning, referential resolution (coref / claim-source / entity), chapter detection, annotation, embedding, entity/quote/claim extraction & shaping (incl. falsifiable-prediction capture), cross-source consensus + contradiction detection. No production pipeline yet — prototyped locally, built as worker passes per [roadmap Track 2](roadmap.md#track-2--interpretive-pass-buildout) (skills local-only — [charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills)). |
...
| **Code**              | `services/api/app/analyst/`. **Legacy (Lineup, externalising — [charter A8](charter.md#a8-disposition-of-the-in-repo-lineup-code--legacy-not-deleted)):** transcription (`transcribe.py`, `diarize.py`, `keyterms.py`), speaker ID (`identify_voice.py`, `visual_id.py`, `fusion.py`), clustering (`face_clusters.py`, `face_runs.py`, `voice_cluster_*.py`, `identity_alignment.py`), remote GPU (`remote.py`, `video_staging.py`, `services/gpu/`), enroll/CLI helpers. **Durable surface:** cleaning / extraction / consensus — no production pipeline yet, prototyped locally (see [Pipeline position](#pipeline-position)), built as workers under [roadmap Track 2](roadmap.md#track-2--interpretive-pass-buildout) ([charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills)). |
...
| Clean | Fix garbles, merge restarts, normalise filler. Writes `source_documents.cleaned_text`, `source_chunks.clean_text`. | Not built in production; local prototype (`/clean-transcript`) — worker pending ([charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills)) |
...
| Extract claims / quotes | LLM extraction over chunks. Writes `quotes`, `claims`, `claim_chunks`. | Not built in production; local prototype (`/process-transcript`) — worker pending ([charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills)) |
...
- **[Extraction](../../system/extraction.md)** — claim / entity resolution, cleaning, augmenting (local prototype today; production worker not built — [charter A11](charter.md#a11-production-runs-in-workers-not-claude-code-skills))
- **[Publishing](../../system/publishing.md)** — `update_consensus_snapshots` for consensus shifts and contradictions (planned)

## Related

- [Charter](charter.md) — locked design decisions A1–A10, the Lineup boundary, risks, cost/testing/rollout
