...
1. **Cleaning, referential resolution, embedding, entity / quote / claim extraction (with falsifiable-prediction capture), cross-source consensus + contradiction detection** — Analyst's durable scope. Resolves what the words point to, cross-references claims across sources, finds contradictions, detects consensus shifts, and makes each prediction *gradeable* for the ledger. Skill-driven today; workerised per [roadmap Track 2](roadmap.md#track-2--interpretive-pass-buildout).
...
| **Scope (durable)**   | The interpretive layer: cleaning, referential resolution (coref / claim-source / entity), chapter detection, annotation, embedding, entity/quote/claim extraction & shaping (incl. falsifiable-prediction capture), cross-source consensus + contradiction detection. Skill-driven today; workerised per [roadmap Track 2](roadmap.md#track-2--interpretive-pass-buildout). |
