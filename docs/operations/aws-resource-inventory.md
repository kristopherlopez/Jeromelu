...
|---|---|---|---|
| `0 18 * * *` | `miner-refresh.sh nrlcom-draw` | Archive current-round nrl.com draw JSON to S3 | 04:00 AEST |
| `15 18 * * *` | `miner-refresh.sh nrlcom-match-centre` | Archive current-round match-centre JSON to S3 | 04:15 AEST |
| `30 18 * * *` | `miner-refresh.sh nrlcom-casualty-ward` | Archive daily casualty-ward snapshot to S3 | 04:30 AEST |
| `45 18 * * *` | `miner-refresh.sh nrlcom-ladder` | Archive current-round ladder JSON to S3 | 04:45 AEST |
| `50 18 * * *` | `miner-refresh.sh nrlcom-stats` | Archive nrl.com stat leaderboards to S3 | 04:50 AEST |
| `20 19 * * *` | `miner-populate.sh nrlcom-current` | Project current-season Miner S3 archives into DB tables inside `jeromelu-api` | 05:20 AEST |
| `45 22 * * *` | `miner-refresh.sh supercoach-roster` | Archive SuperCoach roster and apply SCD-2 people/player-attributes refresh | 08:45 AEST |
| `55 22 * * 0,2,4` | `miner-refresh.sh supercoach-stats current` | Resolve SuperCoach `current_round` and upsert current-round `player_rounds` | Mon/Wed/Fri 08:55 AEST |
| `0 23 * * *` | `miner-refresh.sh channel-stats` | Snapshot subscriber/video/view counts into `channel_metrics` (~3 quota units) | 09:00 AEST / 10:00 AEDT |
| `15 23 * * *` | `miner-refresh.sh videos` | Enumerate new videos per channel + snapshot `video_metrics` (~750 quota units, offset 15 min from channel-stats to avoid DB connection contention) | 09:15 AEST / 10:15 AEDT |
| `30 23 * * 1` | `miner-refresh.sh supercoach-teams` | Enrich `teams.metadata_json.supercoach` | Tue 09:30 AEST |
| `35 23 * * 1` | `miner-refresh.sh supercoach-settings` | Snapshot SuperCoach classic settings into `sc_settings` | Tue 09:35 AEST |
| `40 23 * * 1` | `miner-refresh.sh nrlcom-players-roster` | Archive nrl.com player profile listings for all NRL teams | Tue 09:40 AEST |
...
- miner-refresh → `/var/log/jeromelu/miner-refresh.log`
- miner-populate → `/var/log/jeromelu/miner-populate.log`

`miner-refresh.sh` sources `/opt/jeromelu/.env` for `ADMIN_KEY`, calls the matching admin endpoint, and exits non-zero on non-2xx so cron failures are observable. Box is in UTC — schedules target AEST clock times, so jobs drift +1 hour during AEDT.
