...
| primary_colour | text | yes | | Hex e.g. `#000000` |
| secondary_colour | text | yes | | |
| founded_year | int | yes | | |
| home_venue_id | UUID | yes | | FK → venues (SET NULL) |
| logo_url | text | yes | | |
| metadata_json | jsonb | no | {} | Long-tail (nicknames, fan_club_url, naming_history) |
| active | bool | no | true | |
| created_at | timestamptz | no | now() | |
| updated_at | timestamptz | no | now() | Auto-updates |
...
**FK:** parent_team_id → teams (ON DELETE SET NULL); home_venue_id → venues (ON DELETE SET NULL)
