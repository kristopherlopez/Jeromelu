...
def run_nrlcom_players_roster(
    ...
    *,
    ...
    try:
        ...
        logger.info("miner/nrlcom-players-roster: comp=%s team=%s profiles=%d", competition, team, n_profiles)
...
@router.post(
    "/admin/miner/nrlcom-players-roster",
    ...
    *,
    ...
    for idx, (short, team_id) in enumerate(NRL_TEAM_IDS):
        try:
            ...
            logger.warning(
                "miner/nrlcom-players-roster-refresh-all: team=%s (%s) failed with HTTPException %s: %s",
                team_id,
                short,
                e.status_code,
                e.detail,
            ...
            logger.warning(
                "miner/nrlcom-players-roster-refresh-all: team=%s (%s) failed with %s: %s",
                team_id,
                short,
                type(e).__name__,
                e,
...
@router.post(
    "/admin/miner/nrlcom-players-roster/refresh-all",
    dependencies=[Depends(require_admin)],
)
