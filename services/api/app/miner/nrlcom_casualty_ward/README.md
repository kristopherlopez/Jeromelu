...
| Endpoint | `POST /api/admin/miner/nrlcom-casualty-ward[?season=Y][&competition=N]` |
| Make target | `make miner-nrlcom-casualty-ward [SEASON=2026] [COMPETITION=111]` |
| S3 archive | `miner/nrlcom/casualty-ward/{comp}/{YYYYMMDD}.json` (timestamped — preserves daily history) |
...
- `tests/unit/api/miner/nrlcom_casualty_ward/test_models.py` — always-on D8 drift unit tests against the checked-in fixture (`tests/fixtures/miner/nrlcom_casualty_ward/canonical_response.json`): canonical parse + unknown-top-level / unknown-casualty-field / missing-`teamNickname` negatives. The route strict-parses the response through `NrlcomCasualtyWard` (drift → 500 + failed audit; single-envelope abort, the draw precedent).
- `tests/integration/miner/nrlcom_casualty_ward/test_response_shape.py` — env-flagged (`MINER_DRIFT_LIVE=1`) live drift test against the real `/casualty-ward/data` endpoint. Skipped in CI by default. Per D8 the agent does not auto-adapt — a live shape change fails loudly and routes to the operator.
