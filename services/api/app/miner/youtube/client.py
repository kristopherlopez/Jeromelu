"""YouTube Data API v3 client used by Miner's deterministic discovery tools.

Why a separate module from the existing `youtube_utils.search` package: that
one uses yt-dlp (unauthenticated, no metadata). For Miner we want subscriber
...
Module surface — all functions are sync (Miner's loop is sync) and use httpx
...
def _api_key() -> str:
    ...
    if not key:
        raise YouTubeAPIError("YOUTUBE_API_KEY is not set — Miner's YouTube tools cannot run")
    return key
