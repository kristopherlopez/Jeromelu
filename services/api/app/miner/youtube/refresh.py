...
Two deterministic jobs that bolt on to Miner's discovery flow:
...
Not Temporal-driven — sync, in-process. Matches the rest of Miner.
"""
...
def run_youtube_refresh_videos(
    ...
    *,
    ...
    """Run the daily YouTube refresh endpoint under a Miner agent_runs row."""
...
def run_youtube_channel_videos(
    ...
    *,
    ...
    """Enumerate one channel's uploads under a Miner agent_runs row."""
    run = start_deterministic_run(
        session,
...
def run_youtube_channel_stats(session: Session) -> dict[str, Any]:
    """Run the YouTube channel metrics refresh under a Miner agent_runs row."""
    run = start_deterministic_run(
        session,
