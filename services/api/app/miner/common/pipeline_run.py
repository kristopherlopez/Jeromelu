"""Shared audit helpers for deterministic Miner pipelines.
...
def start_deterministic_run(
    ...
    *,
    ...
    run_id = make_run_id(MINER_AGENT_ID)
    bounds = AgentBounds(
        max_turns=0,
        max_tool_calls=0,
    ...
    record_agent_started(
        db,
        agent_id=MINER_AGENT_ID,
        agent_name=MINER_AGENT_NAME,
        run_id=run_id,
        model=DETERMINISTIC_MODEL,
        brief=brief,
        bounds={**_bounds_dict(bounds), **run_detail},
    )
    return DeterministicMinerRun(
        db=db,
        run_id=run_id,
        pipeline=pipeline,
        detail=run_detail,
