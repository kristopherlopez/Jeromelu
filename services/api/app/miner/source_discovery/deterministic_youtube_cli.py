...
Examples:
  ...
  python -m app.miner.source_discovery.deterministic_youtube_cli --channel-query "NRL injury podcast"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--run-id", help="Optional run_id to stamp onto miner_candidates")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Call YouTube and score candidates but do not write miner_candidates",
    )
    ...
    run_id = args.run_id or make_run_id("miner")
    ...
    if args.json:
        ...
        print(
            ...
            f"below_threshold={result.candidates_below_threshold}, "
            f"missing_metadata={result.candidates_missing_api_metadata}"
        )
