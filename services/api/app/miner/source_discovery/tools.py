"""Miner's tool palette — Anthropic tool definitions + Python handlers.
...
per-run user brief with the channels Miner already knows, so it doesn't
re-discover them.
"""
...
DEDUPE_CHECK_TOOL: dict[str, Any] = {
    ...
    "description": (
        ...
        "candidate in miner_candidates). Call this BEFORE persisting a candidate "
...
def handle_dedupe_check(session: Session, *, kind: str, url: str) -> dict[str, Any]:
    ...
    Checks (in order): channels, sources (for videos), miner_candidates.
    """
    ...
    existing_disc = session.execute(
        select(MinerCandidate.id, MinerCandidate.status).where(
            MinerCandidate.platform == "youtube",
            MinerCandidate.kind == kind,
            MinerCandidate.external_id == external_id,
        )
    ).first()
    if existing_disc:
        return {
            "known": True,
            "where": "miner_candidates",
...
def handle_persist_candidate(
    ...
    *,
    ...
    logger.info("Miner filed candidate kind=%s id=%s title=%s", kind, external_id, title)
    return {
        "ok": True,
        "status": "filed",
...
def summarise_known_sources(session: Session, *, max_lines: int = 200) -> str:
    """Build a compact text summary of YouTube sources Miner already knows.

    Used to seed the per-run user brief. Two sections:
      1. Tracked channels (in `channels`, active or inactive)
      2. Previously surfaced candidates (in `miner_candidates`, any status)
    ...
    surfaced = session.execute(
        select(
            MinerCandidate.title,
            MinerCandidate.external_id,
            MinerCandidate.status,
        )
        .where(
            MinerCandidate.platform == "youtube",
            MinerCandidate.kind == "channel",
        )
        .order_by(MinerCandidate.discovered_at.desc())
    ).all()
...
def _known_video_external_ids(session: Session) -> set[str]:
    """All YouTube video external_ids in miner_candidates (any status)."""
    return {
        row[0]
        for row in session.execute(
            select(MinerCandidate.external_id).where(
