...
hosts the show, files findings into `miner_presenter_candidates` for human
...
def handle_persist_presenter_candidate(
    ...
    *,
    ...
    stmt = (
        pg_insert(MinerPresenterCandidate).values(
        ...
        # The partial unique index uq_miner_pres_channel_name_pending only
    ...
    try:
        result = session.execute(stmt.returning(MinerPresenterCandidate.id)).first()
        session.commit()
    ...
    existing = session.execute(
        select(MinerPresenterCandidate.id, MinerPresenterCandidate.status).where(
            MinerPresenterCandidate.channel_id == channel_id,
            MinerPresenterCandidate.name.ilike(name_clean),
...
def build_brief(
    ...
    *,
    ...
    any presenters already in source_presenters / miner_presenter_candidates
...
def _existing_presenters(session: Session, channel_id: UUID) -> tuple[list[tuple[str, str]], list[tuple[str, str]]]:
    ...
    pending = session.execute(
        select(MinerPresenterCandidate.name, MinerPresenterCandidate.role)
        .where(
            MinerPresenterCandidate.channel_id == channel_id,
            MinerPresenterCandidate.status == "pending",
        )
        .order_by(MinerPresenterCandidate.role, MinerPresenterCandidate.name)
    ).all()
    return (
