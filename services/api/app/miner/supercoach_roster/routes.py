"""Miner/SuperCoach roster admin endpoint with the standard agent_audit wrapper.

POST /api/admin/miner/supercoach-roster
  - Fetches the SC roster server-side (strict-parsed per D8).
  - Runs the SCD-2 refresh from jeromelu_shared.players.roster.
  - Writes an agent_runs row under agent_id='miner', detail_json.pipeline='supercoach-roster'.
...
def run_supercoach_roster(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "miner/supercoach-roster: fetched %d players (season=%s, source=%s, run_id=%s, s3=%s)",
            fetched,
            season,
            source,
            run.run_id,
...
def supercoach_roster_endpoint(
    ...
    per call under `agent_id='miner'`, `detail_json.pipeline='supercoach-roster'`.
    """
