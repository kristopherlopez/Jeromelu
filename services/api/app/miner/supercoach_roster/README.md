...
| Cadence | Daily cron via `scripts/miner-refresh.sh supercoach-roster` at 22:45 UTC (08:45 AEST) |
...
| Endpoint | `POST /api/admin/miner/supercoach-roster` |
| Make target | `make miner-supercoach-roster ADMIN_KEY=$ADMIN_KEY` |
| Drift contract | D8 — strict Pydantic on the structural envelope (`SuperCoachPlayer`, `SCTeam`, `SCPosition`, `SCPlayedStatus`); opaque on `player_stats`/`odds`/`notes` (those are Phase 2's concern via `supercoach_stats`). Fixture: `tests/fixtures/miner/supercoach_roster/canonical_response.json`. |
...
- `routes.py` — `POST /api/admin/miner/supercoach-roster` with the agent_audit wrapper.
...
`POST /api/admin/players/fetch-and-refresh` is the deprecated alias. It calls the same handler. New callers should use `/api/admin/miner/supercoach-roster`.
