"""Read-only Miner dashboard backed by ``agent_runs``."""
...
def build_miner_dashboard(rows: Sequence[Any], *, limit: int) -> dict[str, Any]:
    """Group recent Miner ``agent_runs`` rows by ``detail_json.pipeline``.
...
def _recent_miner_runs(db: Session, *, limit: int) -> list[AgentRun]:
    return (
        db.query(AgentRun)
        .filter(AgentRun.agent_id == MINER_AGENT_ID)
...
@router.get("/admin/miner/dashboard")
def miner_dashboard(
    limit: int = Query(default=500, ge=1, le=2000),
    db: Session = Depends(get_db),
):
    """Recent Miner pipeline health grouped by ``detail_json.pipeline``."""

    rows = _recent_miner_runs(db, limit=limit)
    return build_miner_dashboard(rows, limit=limit)
