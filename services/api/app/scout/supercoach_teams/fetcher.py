...
SUPERCOACH_TEAMS_URL = "https://www.supercoach.com.au/{season}/api/nrl/classic/v1/teams"
...
def fetch_supercoach_teams(
    ...
    if not isinstance(data, list):
        raise SuperCoachTeamsFetchError(f"Unexpected response shape: {type(data).__name__}, expected list")
    if len(data) < 16 or len(data) > 18:
        raise SuperCoachTeamsFetchError(f"Unexpected team count: {len(data)} (NRL has 17)")
    return data


__all__ = ["SuperCoachTeamsFetchError", "fetch_supercoach_teams"]
