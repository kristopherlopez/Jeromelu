"""Scout source health classification.
...
class SourceHealthSummary:
    """Structured Scout source-health output for API/dashboard consumers."""
...
def build_source_health_summary(
    *,
    ...
    """Classify Scout source liveness from already-loaded DB metadata."""
...
def build_source_health_summary_from_db(
    ...
    *,
    ...
    """Load DB metadata and classify Scout source health.
...
def load_pipeline_run_inputs(session: Session, *, limit: int = 200) -> tuple[PipelineRunInput, ...]:
    """Load recent Scout YouTube refresh/backfill rows per watched pipeline.
    ...
    *,
    ...
    if not watched:
        return HealthCheckSummary(
            ...
            status=UNKNOWN,
            summary="No watched YouTube Scout run rows were supplied.",
    ...
    if failed:
        status = FAILED
        summary = "Recent YouTube Scout refresh/backfill failures need operator inspection."
    else:
        status = HEALTHY
        summary = "No recent failed YouTube Scout refresh/backfill runs found."
...
def _source_backlog_check(
    *,
    ...
    if collected_missing_audio or failed_audio or failed_transcription:
        ...
        status = WARN
        summary = "Approved YouTube sources are waiting for Scout audio, Analyst transcription, or caption rework."
    else:
        status = HEALTHY
