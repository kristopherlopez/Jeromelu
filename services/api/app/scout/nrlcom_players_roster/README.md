...
| Per-team endpoint | `POST /api/admin/scout/nrlcom-players-roster?competition=N&team={team_id}` |
| Bulk endpoint | `POST /api/admin/scout/nrlcom-players-roster/refresh-all?competition=N` — walks the 17 NRL teams server-side at 1 req/sec (~20s wall time); non-aborting per-team (errors land in the response body, HTTP status stays 200) |
| Make targets | `make scout-nrlcom-players-roster TEAM=500021` (per-team; e.g. Storm). No make target for refresh-all — cron and operators hit the endpoint directly. |
| S3 archive | `scout/nrlcom/players-roster/{comp}/team-{team_id}.json` (one key per team; overwrites each refresh) |
...
- **Unit (CI):** `pytest tests/unit/api/scout/nrlcom_players_roster/test_models.py` — 4 cases (canonical parse + 3 negatives).
- **Live drift (env-flagged):** `SCOUT_DRIFT_LIVE=1 pytest tests/integration/scout/nrlcom_players_roster/test_response_shape.py` — hits real nrl.com (`team=500011`); fails loudly with the offending field name on drift.
