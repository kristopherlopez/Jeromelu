"""Shared S3 raw-snapshot archive for all Scout pipelines (D10).

Every Scout pipeline calls `archive_response(...)` after a successful upstream
...
Path convention (per D10):
  s3://{s3_clean_bucket}/scout/{source}/{pipeline}/{identity_path}.json
...
SCOUT_S3_PREFIX = "scout"


def build_key(source: str, pipeline: str, identity_path: str) -> str:
    """Compose the S3 key for a Scout snapshot.
    ...
    """
    return f"{SCOUT_S3_PREFIX}/{source}/{pipeline}/{identity_path}"
...
def archive_response(
    *,
    ...
    """Persist the raw upstream response to S3 under the Scout prefix.
