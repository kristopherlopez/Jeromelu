...
| Endpoint | `POST /api/admin/scout/supercoach-stats?round=N&season=Y` |
| Make target | `make scout-supercoach-stats ADMIN_KEY=$ADMIN_KEY ROUND=N [SEASON=2026]` |
| Drift contract | D8 — strict Pydantic on the **extracted** shape (`SuperCoachPlayerStats`, ~58 fields). Catches renames/removals of fields we depend on. Fixture: `tests/fixtures/scout/supercoach_stats/canonical_response.json`. |
