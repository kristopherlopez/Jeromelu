...
interface MinerDashboardWindow {
  limit: number;
  row_count: number;
  pipeline_count: number;
}

interface MinerPipelineRollup {
  pipeline: string;
...
interface MinerDashboardResponse {
  ok: boolean;
  window: MinerDashboardWindow;
  pipeline_order: string[];
  pipelines: Record<string, MinerPipelineRollup>;
}
...
export default function MinerDashboardPanel() {
  const { base } = useAdminApiBase();
  const [limit, setLimit] = useState(500);
  const cacheKey = `${base}:${limit}`;

  const [cache, setCache] = useState<Map<string, MinerDashboardResponse>>(
    () => new Map(),
  );
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${base}/api/admin/miner/dashboard?${params.toString()}`,
      );
      if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
      const json = (await res.json()) as MinerDashboardResponse;
  ...
  const orderedPipelines = useMemo(() => {
    ...
    const ordered = data.pipeline_order
      ...
      .filter((pipeline): pipeline is MinerPipelineRollup => Boolean(pipeline));
  ...
  return (
    <div className="rounded-lg border border-zinc-800">
      <div className="flex flex-wrap items-center gap-3 px-4 py-3">
        <span className="text-sm font-medium text-zinc-300">Miner Dashboard</span>
        {data && (
      ...
      {data && (
        <>
          ...
          {orderedPipelines.length === 0 ? (
            <div className="border-t border-zinc-800 px-4 py-8 text-center text-sm text-zinc-500">
              No Miner pipeline runs in this window.
            </div>
          ) : (
