...
`MINER_DRIFT_LIVE=1`, it resolves a real match from the current draw and
...
import pytest
from app.miner.nrlcom_draw.fetcher import NrlcomDrawFetchError, fetch_draw
from app.miner.nrlcom_match_centre.fetcher import (
...
def test_live_nrlcom_match_centre_shape():
    ...
    try:
        ...
        pytest.fail(
            ...
            f"app.miner.nrlcom_match_centre.models (top-level envelope only — note the "
            f"FullTime/Upcoming state split), regenerate the fixtures under "
            f"tests/fixtures/miner/nrlcom_match_centre/, commit with a note on what "
