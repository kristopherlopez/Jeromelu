...
def test_daily_refresh_failure_marks_run_failed_and_reraises():
    ...
    with (
        patch("app.scout.youtube.refresh.start_deterministic_run", side_effect=fake_start) as mock_start,
        patch("app.scout.youtube.refresh.refresh_all_video_stats", side_effect=error),
    ):
...
def test_per_channel_refresh_success_records_channel_pipeline_detail():
    ...
    with (
        patch("app.scout.youtube.refresh.start_deterministic_run", side_effect=fake_start) as mock_start,
        patch("app.scout.youtube.refresh.refresh_channel_videos", return_value=refresh_result) as mock_refresh,
    ):
    ...
    assert response == {"run_id": "scout-test-run", "ok": True, **refresh_result}
    assert runs[0].completed
...
def test_per_channel_refresh_failure_marks_run_failed_and_reraises():
    ...
    with (
        patch("app.scout.youtube.refresh.start_deterministic_run", side_effect=fake_start) as mock_start,
        patch(
            "app.scout.youtube.refresh.refresh_channel_videos",
...
def test_channel_stats_failure_marks_run_failed_and_reraises():
    ...
    with (
        patch("app.scout.youtube.refresh.start_deterministic_run", side_effect=fake_start) as mock_start,
        patch(
            "app.scout.youtube.refresh.refresh_all_channel_stats",
            side_effect=RuntimeError("channels.list failed"),
        ),
    ):
        with pytest.raises(RuntimeError, match="channels.list failed"):
