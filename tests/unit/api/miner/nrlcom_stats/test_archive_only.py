...
Mirrors `tests/unit/api/miner/nrlcom_draw/test_archive_only.py`.
"""
...
import pytest
from app.miner.nrlcom_stats.routes import run_nrlcom_stats
...
def _patched_run(fixture_payload: dict, *, archive_only: bool):
    ...
    with (
        patch(
            "app.miner.nrlcom_stats.routes.fetch_stats",
            return_value=fixture_payload,
        ),
        patch(
            "app.miner.nrlcom_stats.routes.archive_response",
            return_value="miner/nrlcom/stats/111/2026.json",
        ),
        patch(
            "app.miner.nrlcom_stats.routes.start_deterministic_run",
