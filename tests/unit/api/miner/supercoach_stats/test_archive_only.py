...
import pytest
from app.miner.supercoach_stats.routes import run_supercoach_stats
...
def _patched_run(raw_rows: list[dict[str, Any]], *, archive_only: bool):
    ...
    with (
        patch(
            "app.miner.supercoach_stats.routes.fetch_stats_raw",
            return_value=raw_rows,
        ),
        patch(
            "app.miner.supercoach_stats.routes.archive_response",
            return_value="miner/nrlsupercoachstats/stats/2026/round-12.json",
        ) as mock_archive,
        patch(
            "app.miner.supercoach_stats.routes.start_deterministic_run",
            return_value=fake_run,
        ),
        patch(
            "app.miner.supercoach_stats.routes._upsert_player_rounds",
            return_value=1,
        ) as mock_upsert,
    ):
