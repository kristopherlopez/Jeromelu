...
import pytest
from app.miner.nrlcom_draw.routes import run_nrlcom_draw


class _FakeRun:
    """Minimal stand-in for DeterministicMinerRun (records .complete/.fail)."""
...
def _patched_run(fixture_payload: dict, *, archive_only: bool):
    ...
    with (
        patch(
            "app.miner.nrlcom_draw.routes.fetch_draw",
            return_value=fixture_payload,
        ),
        patch(
            "app.miner.nrlcom_draw.routes.archive_response",
            return_value="miner/nrlcom/draw/111/2026/round-13.json",
        ),
        patch(
            "app.miner.nrlcom_draw.routes.start_deterministic_run",
...
def test_archive_response_called_under_archive_only(drift_payload):
    ...
    with (
        ...
        patch(
            "app.miner.nrlcom_draw.routes.archive_response",
            return_value="miner/nrlcom/draw/111/1995/round-01.json",
        ) as mock_archive,
        patch(
            "app.miner.nrlcom_draw.routes.start_deterministic_run",
