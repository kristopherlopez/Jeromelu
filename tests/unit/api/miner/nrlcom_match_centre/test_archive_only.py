...
import pytest
from app.miner.nrlcom_match_centre.routes import run_nrlcom_match_centre
...
def _patched_walk(*, draw_payload: dict, match_payload: dict, archive_only: bool):
    ...
    with (
        patch(
            "app.miner.nrlcom_match_centre.routes.fetch_draw",
            return_value=draw_payload,
        ),
        patch(
            "app.miner.nrlcom_match_centre.routes.fetch_match_centre",
            return_value=match_payload,
        ),
        patch(
            "app.miner.nrlcom_match_centre.routes.archive_response",
            return_value="miner/nrlcom/match-centre/111/2026/round-12/raiders-v-dolphins.json",
        ),
        patch(
            "app.miner.nrlcom_match_centre.routes.start_deterministic_run",
            return_value=fake_run,
        ),
        patch(
            # Skip the per-match 1-second rate-limit sleep in unit tests
            "app.miner.nrlcom_match_centre.routes.time.sleep",
            return_value=None,
        ),
    ):
