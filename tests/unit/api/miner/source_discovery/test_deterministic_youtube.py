...
from app.miner import routes as miner_routes
from app.miner.source_discovery import deterministic_youtube as dy
from app.miner.source_discovery import deterministic_youtube_cli as dy_cli
...
def test_cli_dry_run_summary_prints_would_insert_count(monkeypatch, capsys):
    ...
    assert dy_cli.main(["--dry-run", "--run-id", "miner-test"]) == 0
...
def test_cli_real_run_summary_preserves_inserted_count(monkeypatch, capsys):
    ...
    assert dy_cli.main(["--run-id", "miner-test"]) == 0
...
def test_miner_router_registers_deterministic_youtube_endpoint():
    route_paths = {getattr(route, "path", None) for route in miner_routes.router.routes}

    assert "/admin/miner/source-discovery/youtube" in route_paths
...
def test_run_filters_known_ids_scores_and_persists_novel_candidates(monkeypatch):
    ...
    assert captured["persist_run_id"] == "miner-test"
