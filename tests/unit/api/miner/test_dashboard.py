"""Unit coverage for the Miner pipeline dashboard read model."""
...
from app.miner.dashboard import build_miner_dashboard, miner_dashboard
...
def test_build_dashboard_groups_recent_rows_by_pipeline_and_preserves_latest_run():
    ...
    response = build_miner_dashboard(rows, limit=500)
...
def test_build_dashboard_handles_null_or_missing_pipeline_detail():
    ...
    response = build_miner_dashboard(rows, limit=50)
...
def test_dashboard_endpoint_queries_recent_miner_runs():
    now = datetime(2026, 5, 30, tzinfo=UTC)
    rows = [
        _run(
            "run-1",
    ...
    response = miner_dashboard(limit=25, db=db)

    db.query.assert_called_once_with(AgentRun)
    filter_arg = query.filter.call_args.args[0]
    assert str(filter_arg.compile(compile_kwargs={"literal_binds": True})) == "agent_runs.agent_id = 'miner'"
