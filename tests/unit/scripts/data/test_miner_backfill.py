"""Phase 5: unit tests for the hardened miner_backfill driver.
...
from scripts.data import miner_backfill
...
def _mock_httpx_client(return_code: int = 200, json_payload: dict | None = None):
    ...
    fake_resp.json.return_value = json_payload or {"run_id": "miner-fake-run"}
...
def test_url_includes_archive_only_when_flag_set():
    ...
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill.time.sleep"):
        exit_code = miner_backfill.main(
...
def test_archive_only_flag_omitted_by_default():
    ...
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill.time.sleep"):
        miner_backfill.main(_common_argv("nrlcom-stats"))
    call_kwargs = fake_client.post.call_args.kwargs
...
def test_resume_skips_when_s3_head_succeeds():
    ...
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill._get_s3_client", return_value=fake_s3), \
         patch("scripts.data.miner_backfill.time.sleep"):
        miner_backfill.main(_common_argv("nrlcom-stats", "--resume"))
...
def test_force_overrides_resume():
    ...
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill._get_s3_client", return_value=fake_s3), \
         patch("scripts.data.miner_backfill.time.sleep"):
        miner_backfill.main(_common_argv("nrlcom-stats", "--resume", "--force"))
...
def test_match_centre_resume_uses_list_prefix():
    ...
    fake_s3.list_objects_v2.return_value = {
        "Contents": [{"Key": "miner/nrlcom/match-centre/111/2026/round-12/foo.json"}],
    }
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill._get_s3_client", return_value=fake_s3), \
         patch("scripts.data.miner_backfill.time.sleep"):
        # Single round so we only iterate once
        miner_backfill.main(_common_argv(
            "nrlcom-match-centre",
...
def test_unknown_source_exits_2():
    ...
    exit_code = miner_backfill.main(_common_argv("not-a-real-source"))
    assert exit_code == 2
...
def test_supercoach_roster_resume_is_noop():
    ...
    with patch("scripts.data.miner_backfill.httpx.Client", return_value=fake_client), \
         patch("scripts.data.miner_backfill._get_s3_client", return_value=fake_s3), \
         patch("scripts.data.miner_backfill.time.sleep"):
        miner_backfill.main(_common_argv("supercoach-roster", "--resume"))
    assert fake_s3.head_object.call_count == 0
